...
```bash
test-node-typescript$ sam build
```
...
```bash
test-node-typescript$ sam local invoke HelloWorldFunction --event events/event.json
```

The SAM CLI can also emulate your application's API. Use the `sam local start-api` to run the API locally on port 3000.

```bash
test-node-typescript$ sam local start-api
test-node-typescript$ curl http://localhost:3000/
```
...
```bash
test-node-typescript$ sam logs -n HelloWorldFunction --stack-name test-node-typescript --tail
```
...
```bash
test-node-typescript$ cd hello-world
...
```bash
sam delete --stack-name test-node-typescript
```
